...
type Match<ItemType> = { _matchScore?: number } & ItemType

export function matchQuery<ItemType>(
  ...
  return items.reduce((matches: Array<Match<ItemType>>, item) => {
    ...
    // This is the score that is given to the current query. It represents how well
    // the query matches the current item.
    let queryScore = 0
    ...
    for (const termWord of searchTerms) {
      ...
      if (query.length > termWord.length) {
        continue
      }
      ...
      if (termWordScore > 0) {
        queryScore += termWordScore
      }
    }

    // 10 is the threshold for an item being included in the result array.
    // It's a bit random, adjust if necessary.
    if (queryScore > 10) {
      // @ts-ignore
      item._matchScore = (item._matchScore || 0) + queryScore
      matches.push(item)
    }
...
export function search<ItemType>(
  ...
  let filteredItems: Array<Match<ItemType>> = items
